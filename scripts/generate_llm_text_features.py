...
def main():
    ...
    print("\n3. 正在编码并保存向下兼容的密集语义特征 .pt 文件...")
    # ==========================================
    # 兼容性设定：匹配 configs.py 中的 text_group_summary_count
    # ==========================================
    ...
    for domain, text in domain_texts.items():
        ...
        with torch.no_grad():
            ...
            # 提取句子本身的密集细粒度特征 (去掉 [CLS] 和最后的 [SEP])
            fine_feats = valid_feats[1:-1]                  
            
        # ==========================================
        # 结构伪装：桥接 LLM 特征与现有的 Dense Guidance 架构
        # ==========================================
        # 复制全局特征 5 次，充当原本框架需要的 Group Summary
        group_summary_tokens = global_feat.repeat(GROUP_SUMMARY_COUNT, 1) # [5, 1024]
        
        # 拼接成最终模型期望的格式: 前 5 个是 summary, 后面跟着密集子词特征
    ...
    print("\n✅ 1024维安全格式密集文本特征生成完毕，且已完全适配 Dense Guidance 架构！")
